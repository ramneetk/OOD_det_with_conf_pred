...
class Regressor(nn.Module):
    def __init__(self, depth=40, widen_factor=2, dropRate=0.3, indim=256, num_classes=8, dropout = None): #indim = [orig_img_features,transformed_img_features], num_classes is the number of possible transformations
    ...
    def forward(self, x1, x2, out_feat_keys=None):
        x1 = self.wrn(self.dropout_layer(x1), out_feat_keys)
        x2 = self.wrn(self.dropout_layer(x2), out_feat_keys)
        if out_feat_keys==None:
            x = self.dropout_layer(torch.cat((x1,x2), dim=1))
